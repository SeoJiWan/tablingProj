...
public class Storereservation implements Controller {
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		String stroeName = store.getStoreName();
		
		StoreService service = StoreService.getInstance();
		List<Reservations> pageList = service.getstorereservations(cri, stroeName);
		
		req.setAttribute("list", pageList);			
		
		List<Reservations> totalList = service.reservationList(stroeName);
